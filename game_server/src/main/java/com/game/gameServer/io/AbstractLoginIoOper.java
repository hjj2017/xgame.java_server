...
public abstract class AbstractLoginIoOper implements IBindUIdIoOper, IMsgDispatchable {
    ...
    protected static long getBindUIdByPlayer(Player p) {
        ...
        if (p._platformUIdStr.length() == 1) {
            // 如果平台 UId 就一个字符,
            // 则直接返回!
            return p._platformUIdStr.charAt(0);
        }

        // 获取字符串长度
        int strLen = p._platformUIdStr.length();
        // 获取最后两位字符
        long n1 = p._platformUIdStr.charAt(strLen - 2);
        long n0 = p._platformUIdStr.charAt(strLen - 1);
        // 换算为 10 进制数
        return n1 * 10 + n0;
    }
}
